...
import com.testapp.test.QualityBizneGroupActivity;
import com.testapp.test.QualityGroupActivity;

public class GetBizneGroup extends AsyncTask <String, Void, String> {
	...
	public static AnswerBizneGroup act;
	boolean opengroup = false;

	public GetBizneGroup (Activity act, ProgressDialog tempd){
	...
	public GetBizneGroup (Activity act, ProgressDialog tempd, boolean opengroup){
	...
	protected String doInBackground(String... params) {
		...
		Log.d("SocialGo", "GetBizneGroup-doInBackground: Enviando datos para login del usuario al servidor");
		String response = "";
		try {
		...
		try {
			...
			Log.d("SocialGo", "GetBizneGroup-doInBackground: Decodificando respuesta");
			
			AnswerBizneGroup answer = gson.fromJson(response.toString(), AnswerBizneGroup.class);
			GetBizneGroup.act = answer;
			...
			if(jError.getMsg().equals("Erro al cargar la lista")){
				...
				Log.e("SocialGo", "GetBizneGroup-doInBackground: No existe la lista de negocios");
				Toast.makeText(this.activity, "No hay actividad asignada" , Toast.LENGTH_SHORT).show();
				isSuccessfull = false;
			}else{
				result = "Communication error...";
				Log.e("SocialGo", "GetBizneGroup-doInBackground: fallo el envio del registo");
	...
	Log.d("SocialGo", "GetBizneGroup-doInBackground: "+result.toString());
	...
	try{
		...
		if(isSuccessfull){
			...
			if(opengroup){
				Intent intent;
				intent = new Intent(activity, QualityBizneGroupActivity.class);
				activity.startActivity(intent);
			}else{
				Intent intent;
				intent = new Intent(activity, QualityGroupActivity.class);
				activity.startActivity(intent);
			}
		  ...
		  Log.e("SocialGo", "GetBizneGroup-doInBackground: Error cargando actividad"+ e.getMessage());
		  pd.dismiss();
	  }
