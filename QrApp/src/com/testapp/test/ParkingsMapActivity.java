...
public class ParkingsMapActivity extends FragmentActivity {
    ...
    public static Parking selected_park = null;
    
    public List<Parking> parkings;
	
	public ParkingsMapActivity() {
		// TODO Auto-generated constructor stub
//		this.parkings = GetParkings.answer.getData();
	}
...
//            setZoom();
	...
	public void setZoom(List<Parking> parkings){
		
		if(parkings.size() == 1){
			googleMap.moveCamera( CameraUpdateFactory.newLatLngZoom(new LatLng(parkings.get(0).getLatitude(),parkings.get(0).getLongitude()) , 15) );
			return;
		}
		...
		for(int i=0; i < parkings.size(); i++){
			LatLng point = new LatLng(parkings.get(i).getLatitude(), parkings.get(i).getLongitude());
			b.include(point);
		}
	...
	private void openParking(Marker marker){
		for(int i = 0; i < parkings.size(); i++){
			if(marker.getTitle().equals(parkings.get(i).getName())){
				selected_park = parkings.get(i);
				Intent a = new Intent(this,ParkingActivity.class);
		        startActivity(a);
				break;
			}
		}
    ...
    private void setMarkers(){
    	...
    	for(int i = 0; i < parkings.size(); i++){
    		// create marker
        	MarkerOptions marker = new MarkerOptions().position(new LatLng(parkings.get(i).getLatitude(), parkings.get(i).getLongitude())).title(parkings.get(i).getName()).icon(BitmapDescriptorFactory.fromResource(R.drawable.parking_lot));
        	// adding marker
